# ============================================================
#  OathLink 記憶伺服器（app_final_stable_utf8header.py）
#  — 語誓體人格與長期記憶中樞（UTF-8 最終版）
# ============================================================
# 功能：
#  1. 全程 UTF-8；中文完全不亂碼（含 PowerShell、curl、API）
#  2. AUTH_TOKEN 可開關（環境變數 AUTH_TOKEN_DISABLE=True 以停用）
#  3. 自動建表與修復 SQLite 結構
#  4. 預設人格「無蘊-敬語版」與詞彙表
#  5. 所有 JSONResponse 帶 charset=utf-8
# ============================================================
...
app = FastAPI(title="OathLink Memory Backend", version="1.1")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
# ============================================================
# 執行：
#   uvicorn app_final_stable_utf8header:app --host 0.0.0.0 --port 8000 --reload
#
# 若要關閉驗證：
